...
class Attention_dot_te(torch.nn.Module):
    ...
    def __init__(self, in_channels: int, head_channels: int):
        ...
        super().__init__()
        self.norm = te.LayerNorm(in_channels)
...
class MLP(torch.nn.Module):
    def __init__(self, channels: int, expansion: int):
        super().__init__()
        self.norm = te.LayerNorm(channels)
        self.main = torch.nn.Sequential(
            te.Linear(channels, channels * expansion),
            torch.nn.GELU(),
            te.Linear(channels * expansion, channels),
        )
...
class AttentionBlock(torch.nn.Module):
    def __init__(self, channels: int, head_channels: int, expansion: int = 4):
        ...
        self.ln_mlp = te.LayerNormMLP(channels, channels, eps=1e-8, bias=True)
...
class MetaBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.proj_in = te.Linear(in_channels, channels)
        ...
        self.proj_out = te.Linear(channels, output_dim)
