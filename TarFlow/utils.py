...
def get_data(
    dataset: str, img_size: int, folder: pathlib.Path
...
def from_255_to_one_minus_one(data_path):
    """
    Args:
        data_path (str or Path): Path to folder containing images.
    Returns:
        torch.Tensor: Tensor of shape (N, C, H, W) with pixel values in [-1,1], dtype=torch.float64.
    """
    data_path = Path(data_path)
    img_files = list(data_path.glob("*.*"))  # get all files in the folder
    imgs = []

    # transformation: PIL -> Tensor -> normalize to [-1,1]
    transform = transforms.Compose(
        [
            transforms.ToTensor(),  # converts from [0,255] to [0,1], dtype float32
            transforms.Lambda(
                lambda x: (x * 2 - 1).to(torch.float64)
            ),  # scale to [-1,1] and cast to float64
        ]
    )

    for f in img_files:
        img = Image.open(f).convert("RGB")  # force 3 channels
        img_tensor = transform(img)
        imgs.append(img_tensor)

    return torch.stack(imgs)  # shape: (N, C, H, W), dtype=torch.float64
