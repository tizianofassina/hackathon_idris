...
# ============================================================
## Setting up Fully Sharded Data Parallel (FSDP)
# ============================================================
...
def setup_fsdp():
    ...
    """
    dist.init_process_group(backend="nccl")  # GPU-to-GPU communication backend
...
def main(local_rank):
    ...
    # ============================================================
    ## 🏗️ Model Setup with FSDP
    # ============================================================
    # Build the model in fp32 on the local device first
    model = Model(
    ...
    CKPT_FILE = None
    if is_main:
        RUN_NAME = f"model_fsdp_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # No autocast: FSDP MixedPrecisionPolicy handles bf16 casting itself
    ...
    # ============================================================
    ## 💾 Checkpoint (rank 0 only, with full state dict)
    # ============================================================
    # Gather the full (unsharded) state dict to rank 0 for checkpointing
    state_dict_options = StateDictOptions(
        full_state_dict=True,
        cpu_offload=True,
    )
    full_state_dict = get_model_state_dict(model, options=state_dict_options)
