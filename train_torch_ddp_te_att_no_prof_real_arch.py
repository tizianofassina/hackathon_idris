...
def main(local_rank, batch_size):
    ...
    if dist.get_rank() == 0:
        with open("total_times.txt", "a") as f:
            f.write(f"te with attention real arch, without fp8, fused mlp| batch_size={BATCH_SIZE} | last_loss={avg_loss}\n")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if dist.get_rank() == 0:
            ...
            with open("total_times.txt", "a") as f:
                f.write(f"te with attention real arch, without fp8, fused mlp| batch_size={args.batch_size} | time={elapsed_time:.2f}\n")
    finally:
        cleanup_ddp()